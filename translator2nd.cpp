	...
	for(int j=(xmin);j<(ymax2);j=j+dx){
		for(int i=(xmin);i<(xmax2);i=i+dx){
			...
			if(y==1){
				...
				//Boundary (2)
				double M_1,M_2,M_0;
				M_2=uplus[(xmin+3)][i]-uplus[(xmin+2)][i];
				if(round(M_2)!=0){
					M_1=uplus[(xmin+2)][i]-uplus[(xmin+1)][i];
					M_0=(M_1*M_1)/round(M_2);
					uplus[xmin][i]=uplus[(xmin+1)][i]-M_0;
				}
				
				M_2=uplus[(ymax-3)][i]-uplus[(ymax-2)][i];
				if(round(M_2)!=0){
					M_1=uplus[(ymax-2)][i]-uplus[(ymax-1)][i];
					M_0=(M_1*M_1)/round(M_2);
					uplus[ymax][i]=uplus[(ymax-1)][i]-M_0;
				}
				
				M_2=uplus[j][(xmin+3)]-uplus[j][(xmin+2)];
				if(round(M_2)!=0){
					M_1=uplus[j][(xmin+2)]-uplus[j][(xmin+1)];
					M_0=(M_1*M_1)/round(M_2);
					uplus[j][xmin]=uplus[j][(xmin+1)]-M_0;
				}
				
				M_2=uplus[j][(xmax-3)]-uplus[j][(xmax-2)];
				if(round(M_2)!=0){
					M_1=uplus[j][(xmax-2)]-uplus[j][(xmax-1)];
					M_0=(M_1*M_1)/round(M_2);
					uplus[j][xmax]=uplus[j][(xmax-1)]-M_0;
				}
			}
