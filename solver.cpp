...
int trivialMatrix(char* filename, double tolerance, bool infinite,double max) {
  ...
  */
  ...
  for(int j=xmin;j<ymax2;j=j+dx){
	...
	cout << "Error tolerance reached: " << std  << " " << (10000-tmax)/dt << " iterations"<< endl;
	break;
      }
  ...
  for(int j=0;j<(ymax2-2*shifty);j=j+3*(stepy)*dx){
    for(int i=0;i<(xmax2-2*shiftx);i=i+3*(stepx)*dx){
      grad << xvalues[i] << " " << yvalues[j] << " " << ugradx[j][i]*sqrt(height*height+width*width)/(3*max)<< " " << ugrady[j][i]*sqrt(height*height+width*width)/(3*max) << endl;
    }
    grad << " " << endl;
  }
  grad.close();
